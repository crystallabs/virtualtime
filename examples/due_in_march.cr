...
# Create a VirtualTime that matches every other
...
p item.on?( Crystime::VirtualTime["2017-02-15"])

# Item is not due on Mar 15, 2017 because that's not a day of
# March 10, 12, 14, 16, 18, or 20:
p item.on?( Crystime::VirtualTime["2017-03-15"])

# But item is due on Mar 16, 2017:
p item.on?( Crystime::VirtualTime["2017-03-16"])

# Also it is due on Mar 20, 2017:
p item.on?( Crystime::VirtualTime["2017-03-20"])

# And it is due on any Mar 20, doesn't need to be in 2017:
any_mar_20 = Crystime::VirtualTime.new
any_mar_20.month = 3
any_mar_20.day = 20
